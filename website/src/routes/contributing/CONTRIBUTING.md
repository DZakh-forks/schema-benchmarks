...
6. Add a `types` config to the benchmark definitions if the library infers TypeScript types from its schemas, naming the imports the type probe needs and the expressions its input and output types are read with. Add its `fromType` entry too when the library can build a schema from a type that already exists.
